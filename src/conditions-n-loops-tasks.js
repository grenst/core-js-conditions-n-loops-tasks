 ...
 *  -0.1, 0, 0.2  => 0.2
 */
function getMaxNumber(numA, numB, numC) {
  let biggernum;
...
function canQueenCaptureKing(queen, king) {
  ...
  axisX = queen.x === king.x;
  axisY = queen.y === king.y;
  diagonal = Math.abs(queen.x - king.x) === Math.abs(queen.y - king.y);

  return axisX || axisY || diagonal;
}
...
function isPalindrome(str) {
  ...
  if (str.length % 2 === 0) {
    ...
    for (let i = end - 1; i >= str.length / 2; i -= 1) {
      seconStr += str[i];
    }
  }
  ...
  if (str.length % 2 !== 0) {
    ...
    for (let i = end - 1; i > Math.floor(str.length / 2); i -= 1) {
      seconStr += str[i];
    }
  }

  return seconStr === firstStr;
}
